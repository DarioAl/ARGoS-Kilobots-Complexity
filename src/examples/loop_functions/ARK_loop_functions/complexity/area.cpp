...
bool AreaALF::doStep(std::string exploitation_type, Real discretization) {
  ...
  if(exploitation_type == "cubic") {
    ...
    std::cout << "exploitation is " << population*lambda*pow(kilobots_in_area, 2)*discretization << std::endl;
  } else {
  ...
  std::cout <<"growth is " << population*eta*(1-population)*discretization << std::endl;
